# AGENTS.md — Mood Monster Shelter(心情怪兽收容所)

本文件是项目的常驻指令,每次会话 Codex 会自动读取。请严格遵守下列技术栈、架构原则与产品约束。

## SDD 开发流程（2026-08-26 起）

- 用户已要求整个开发流程遵循规格驱动开发，先阅读 `specs/README.md`。
- 当前已确认交付为 **电脑端核心流程预览**，先读 `specs/002-desktop-preview/spec.md`；使用本地规则、保持小程序风格。`specs/001-web-friends-beta/spec.md` 的真实 API 与在线部署留待后续。
- 按需求规格 → 技术计划 → 任务 → 实现 → 验收执行；需求、任务与验收案例必须可追溯，不先改业务代码再补规格。
- Web 交付沿用现有 Taro H5、React、TypeScript、Less 和 CloudBase，保留小程序代码与构建，不代表获准重写为 Next.js。
- 新功能规格未确认前可以研究和编制草案；不得把草案、模拟响应、历史测试记录当作已实现或上线证据。
- Key 只在服务端配置，不写入聊天、规格、浏览器包或日志；公开模型访问和费用相关配置需用户确认。
...
- 涉及页面 UI、字体、品牌标题、按钮、卡片、导航、间距或手机适配的开发前，必须先阅读 `doc/mood-monster-visual-design-guidelines-v0.1.md`。
- 首页品牌标题固定使用 `src/assets/brand/home-wordmark.png`，不得用系统字体、CSS 文字或逐字变形重新绘制。
- 有底部菜单的页面必须使用 `TabPageLayout`，页面内容可滚动，但底部菜单必须始终锁定在屏幕底部。
- 气泡和绝对定位元素不得与后续卡片重叠；修改后必须同时验证窄屏、紧凑屏和标准手机视口。
- 视觉方向或关键组件发生变化时，必须与实现代码在同一次修改中更新视觉规范。
...
**技术栈保持 Taro + React + TypeScript + CloudBase 云开发。** 既有路线为微信小程序优先；2026-08-26 新增 Web 朋友内测交付目标，沿用同一代码库的 H5 能力，具体范围按 SDD 规格评审。

| 层级 | 选型 |
|---|---|
| 端形态 | 微信小程序与 H5；本次新增 Web 朋友内测交付，保留小程序构建，不做原生 App |
...
**原选型依据:** 早期侧重微信内分享传播，选择小程序；当前补齐可通过链接访问的 Web 内测版，复用 Taro + React 的多端能力，行动闭环与安全边界不变。
